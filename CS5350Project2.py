...
def int_to_Char_Array2(_attributes, _labels):
    _matrix = []
...
def int_to_Char_Array(_attributes, _labels):
    ...
    for x in range(300):
        if foldC:
            z = random.randint(0, 6075)
        else:
            z = random.randint(0, 7596)
        _new_row = []
        for y in range(len(_attributes[z].flat)):
            if y == 0:
                if _labels[z] == 0:
                    _new_row.append('p')
                else:
                    _new_row.append('e')
                _new_row.append(labelDivider(_attributes[z].flat[y]))
            else:
                _new_row.append(labelDivider(_attributes[z].flat[y]))
        _matrix.append(_new_row)
    return _matrix
...
for x in range(w):
    ...
    trainData_obj = Data(data=trainData)
    attributesSet = trainData_obj.attributes
    root = id3Depth(attributesSet, trainData_obj, 3) # 3 2 1 2
    _root.append(root)
...
with open('IDs') as csvfile:
    ...
    with open('result1.csv', mode='w') as employee_file:
        ...
        for row in readCSV:
            ...
            eCount = 0;
            for x in range(w):
                expected = expectedLabel(accuracyData_obj.attributes, rows[counter], _root[x])
                if expected == 'p':
                    pCount = pCount + 1
                else:
                    eCount = eCount + 1
